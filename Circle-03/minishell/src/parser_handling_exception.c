...
/*   Updated: 2022/05/09 20:34:51 by dason            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	check_parse_error_near(t_list *list)
{
	...
	{
		if (list->next->l_type == LTYPE_PIPE)
		{
		...
		{
			if (list->next->l_type == LTYPE_REDIRECT_L1)
				printf("minishell: syntax error near unexpected token `<'\n");
			if (list->next->l_type == LTYPE_REDIRECT_L2)
				printf("minishell: syntax error near unexpected token `<<'\n");
			if (list->next->l_type == LTYPE_REDIRECT_R1)
				printf("minishell: syntax error near unexpected token `>'\n");
			if (list->next->l_type == LTYPE_REDIRECT_R2)
				printf("minishell: syntax error near unexpected token `>>'\n");
			return (false);
		}
	}
